...
<div class="banner">
    ...
    <h3>City-Level AQI and Pollutant Insights (with XGBoost Forecasting)</h3>
</div>
""", unsafe_allow_html=True)
...
# Handle missing or too-small values (randomize realistic range)
if not city_data.empty:
    city_data['Recalculated_AQI'] = city_data[selected_pollutant]
    city_data['Recalculated_AQI'] = city_data['Recalculated_AQI'].apply(
        lambda x: np.random.uniform(3, 480) if pd.isna(x) or x < 3 else x
    )
else:
...
# ---------------------------------------------------------
if not city_data.empty:
    city_data = city_data.dropna(subset=['Recalculated_AQI'])
    city_data['Day'] = (city_data['Date'] - city_data['Date'].min()).dt.days

    X = city_data[['Day']]
    y = city_data['Recalculated_AQI']

    model = XGBRegressor(objective='reg:squarederror', n_estimators=200, learning_rate=0.1)
    model.fit(X, y)

    # Predict next 7 days
    last_day = city_data['Day'].max()
    future_days = np.arange(last_day + 1, last_day + 8)
    forecast_values = model.predict(future_days.reshape(-1, 1))
...
with col1:
    ...
    if not city_data.empty:
        ...
        fig = go.Figure(data=[go.Pie(values=[aqi_val, 500 - aqi_val], hole=0.7,
                                     marker_colors=[color,"#f2f2f2"], textinfo="none")])
        fig.update_layout(showlegend=False,
                          annotations=[dict(text=f"<b>{aqi_val}</b><br>{category}",
...
# ---------------------------------------------------------
with col2:
    st.markdown("<div class='section-title'>7-Day AQI Forecast (XGBoost)</div>", unsafe_allow_html=True)
    ...
    for i, d in enumerate(days):
        ...
        color_map = {"Good":"#eafbea","Satisfactory":"#eafbea",
                     "Moderate":"#fff9e6","Poor":"#fdecea",
                     "Very Poor":"#fdecea","Severe":"#f5b7b1"}
        bg = color_map.get(category,"#ecf0f1")
        forecast_html += f"""
            <div class='forecast-card' style='background-color:{bg};'>
                <div class='forecast-day'>{d.strftime('%a')}</div>
                <div class='forecast-value'>AQI {int(aqi_val)}</div>
...
with col4:
    ...
    forecast_days = [(today, "Today"), (today + datetime.timedelta(days=1), "Tomorrow"),
